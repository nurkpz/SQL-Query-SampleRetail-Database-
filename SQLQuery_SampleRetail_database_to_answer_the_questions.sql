...
SELECT order_id, COUNT(quantity) AS quantity_of_products 
...
--8. Compare the results of these two queries above. How are the results different when you switch the column you sort on first? (Explain it in your own words.)
  ...
  --in the first query, the list_prices were sorted descending for each brand_id sorted by ascending 
  --but in the second query list_price is sorted descending and list_price same ones are sorted by ascending brand_id
...
SELECT * FROM sale.customer WHERE last_name LIKE 'B%' AND last_name LIKE '%s' ;

SELECT * FROM sale.customer WHERE last_name LIKE 'B%s' ;
